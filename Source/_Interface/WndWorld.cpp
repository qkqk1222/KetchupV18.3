...
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
	int nBlend = 255;
	if (m_vDestinationArrow == D3DXVECTOR3(-1.0F, 0.0F, -1.0F)
#ifndef __KIA_GOALDATA_FIX
		|| g_pPlayer->GetWorld()->GetID() != WI_WORLD_MADRIGAL
#endif
		)
		nBlend = 0;
	else
...
/*#if __VER >= 8 // __CSC_VER8_1
						...
						if (pMover->IsAuthHigher(AUTH_JOURNALIST))
							sprintf(szText, "%s [STAFF]", pMover->GetName());
						else if (pMover->IsLegendHero())
							sprintf(szText, "%s [Lvl: %d-L]", pMover->GetName(), pMover->GetLevel());
						else if (pMover->IsHero())
		...
		case MMI_TRADE: //  거래 
			{
				...
				if( pProp )
				{
					if( pProp->nGrade == -6 )					
					{
						g_WndMng.PutString( "You wont be able to use the store", NULL, prj.GetTextColor( TID_GAME_REPAIR_NOACTION ) );
						break;
					}
				}
#endif // __VER < 8 // __S8_PK
...
void CWndWorld::RenderBuffIcon( C2DRender *p2DRender, IBuff* pBuff, BOOL bPlayer, BUFFICON_INFO* pInfo, CPoint ptMouse )
{
	...
	if( rc.PtInRect( ptMouse ) )
	{
		...
		switch( pItem->dwID )
		{
			...
			case II_SYS_SYS_SCR_NOR_SMELTING01:
#endif
#ifdef __SYS_SCR_NOR_SMELTING02
			case II_SYS_SYS_SCR_NOR_SMELTING02:
#endif
#ifdef __SYS_SCR_NOR_SMELTING03
			case II_SYS_SYS_SCR_NOR_SMELTING03:
#endif
...
//	int nTexture = 0;
	...
	CEditString strEdit;
	strEdit.AddString( "Scroll of Amplification" , 0xff0000ff, ESSTY_BOLD );
	strEdit.AddString( '\n' );
...
#endif // __EXT_ATTRIBUTE
#ifdef __SYS_SCR_NOR_SMELTING01
					|| dwSkillID == II_SYS_SYS_SCR_NOR_SMELTING01
#endif
#ifdef __SYS_SCR_NOR_SMELTING02
					|| dwSkillID == II_SYS_SYS_SCR_NOR_SMELTING02
#endif
#ifdef __SYS_SCR_NOR_SMELTING03
					|| dwSkillID == II_SYS_SYS_SCR_NOR_SMELTING03
#endif
#ifdef __DST_GIFTBOX
					|| wType == BUFF_EQUIP
#endif // __DST_GIFTBOX
#if __VER >= 19
					|| pItem->dwItemKind2 == IK2_ONCE
#endif
					)
					bTime = FALSE;
...
{	
	...
	if (dwid == APP_CHARACTER3)
		...
		((CWndButton*)pWndBase)->SetTexture(m_pParentWnd->m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "Alert_Notice.bmp"), TRUE);
