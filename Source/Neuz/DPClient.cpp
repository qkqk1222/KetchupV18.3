...
#ifdef __ITEMLINK
			case SNAPSHOTTYPE_ITEMLINK:	OnItemLink( ar ); break;
#endif
#ifdef __LOOKCHANGE
...
void CDPClient::OnReplace( CAr & ar )
{
...
#endif // __GUILD_COMBAT_1TO1
	{
	...
	else if( dwWorldID == WI_WORLD_ARENA || dwWorldID == WI_WORLD_ARENA_1)
		...
		strPath = MakePath( DIR_THEME, "MapLoading_Colosseum.tga" );
		...
		strPath = MakePath( DIR_THEME, "MapLoading_ herneos.tga" );
		...
		strPath = MakePath( DIR_THEME, "MapLoading_Market.tga" );
	else if( dwWorldID == WI_WORLD_FWC )
		strPath = MakePath( DIR_THEME, "MapLoading_Arena.tga" );
	else if (dwWorldID == WI_INSTANCE_CONTAMINTRAILS )
		strPath = MakePath(DIR_THEME, "MapLoading_RisisContamin.tga");
...
#ifdef __V21_LEVELUP_SFX
			if( pMover->IsInteriorityJob(JOB_MERCENARY) )
			{ CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK01, pMover->GetPos(), pMover->GetId() ); }
			else if( pMover->IsInteriorityJob(JOB_MAGICIAN) )
			{ CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK05, pMover->GetPos(), pMover->GetId() ); }
			else if( pMover->IsInteriorityJob(JOB_ASSIST) )
			{ CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK02, pMover->GetPos(), pMover->GetId() ); }
			else if( pMover->IsInteriorityJob(JOB_ACROBAT) )
			{ CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK06, pMover->GetPos(), pMover->GetId() ); }
			else
			{ CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK03, pMover->GetPos(), pMover->GetId()); }
#else
...
void CDPClient::OnSetExperience( OBJID objid, CAr & ar )
{
	...
	if( IsValidObj( (CObj*)pMover ) )
	{
		...
		{
#ifdef __V21_LEVELUP_SFX
			if (g_pPlayer->IsInteriorityJob(JOB_MERCENARY))
			{
				CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK01, g_pPlayer->GetPos(), g_pPlayer->GetId());
			}
			else if (g_pPlayer->IsInteriorityJob(JOB_MAGICIAN))
			{
				CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK05, g_pPlayer->GetPos(), g_pPlayer->GetId());
			}
			else if (g_pPlayer->IsInteriorityJob(JOB_ASSIST))
			{
				CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK02, g_pPlayer->GetPos(), g_pPlayer->GetId());
			}
			else if (g_pPlayer->IsInteriorityJob(JOB_ACROBAT))
			{
				CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK06, g_pPlayer->GetPos(), g_pPlayer->GetId());
			}
			else
			{
				CreateSfx(g_Neuz.m_pd3dDevice, XI_SYS_HEROMARK03, g_pPlayer->GetPos(), g_pPlayer->GetId());
			}
#else
...
void CDPClient::OnGameJoin( CAr & ar )
{
	...
	DWORD dwSavePlayerTime = g_WndMng.m_dwSavePlayerTime / 60;
	CString strMessage;
...
void CDPClient::SendDoUseItem( DWORD dwItemId, OBJID objid, int nPart, BOOL bResult )
{
	...
	if (bResult && pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING01)
	{
		SAFE_DELETE(g_WndMng.m_pWndCommItemDlg);
		g_WndMng.m_pWndCommItemDlg = new CWndCommItemDlg;
		g_WndMng.m_pWndCommItemDlg->Initialize(&g_WndMng, APP_COMMITEM_DIALOG);
	...
	if (bResult && pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING02)
	{
		SAFE_DELETE(g_WndMng.m_pWndCommItemDlg);
		g_WndMng.m_pWndCommItemDlg = new CWndCommItemDlg;
		g_WndMng.m_pWndCommItemDlg->Initialize(&g_WndMng, APP_COMMITEM_DIALOG);
	...
	if (bResult && pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING03)
	{
		SAFE_DELETE(g_WndMng.m_pWndCommItemDlg);
		g_WndMng.m_pWndCommItemDlg = new CWndCommItemDlg;
		g_WndMng.m_pWndCommItemDlg->Initialize(&g_WndMng, APP_COMMITEM_DIALOG);
	...
	if( bResult )
	{
		switch( pItemProp->dwID )
		{
			...
			case II_GEN_TOO_COL_SILVERBATTERY_P:
			case II_GEN_TOO_COL_GOLDBATTERY_P:
			case II_GEN_TOO_COL_BATTERY001_P:
#endif
				SAFE_DELETE( g_WndMng.m_pWndCommItemDlg );
				g_WndMng.m_pWndCommItemDlg = new CWndCommItemDlg;
				g_WndMng.m_pWndCommItemDlg->Initialize( &g_WndMng, APP_COMMITEM_DIALOG );
...
void CDPClient::SendReqNPCPos(const unsigned long questId)
{
...
void CDPClient::OnNPCPos( CAr & ar )
{
	...
	{
		const LPCHARACTER npc = prj.GetCharacter(qProp->m_szEndCondCharacter);
		if (npc)
			npc->m_vPos = vPos;
		qProp->m_MeetCharacterGoalData.m_fGoalPositionX = vPos.x;
		qProp->m_MeetCharacterGoalData.m_fGoalPositionZ = vPos.z;
		qProp->m_MeetCharacterGoalData.worldId = worldId;
	}
#else
	CWndWorld* pWndWorld = dynamic_cast<CWndWorld*>(g_WndMng.GetWndBase(APP_WORLD));
	if (pWndWorld)
	{
...
#endif // __COLOSSEUM
#ifdef __ITEMLINK
void CDPClient::SendItemLink( DWORD dwPlayerId, DWORD dwObjId )
{
	BEFORESENDSOLE( ar, PACKETTYPE_ITEMLINK, DPID_UNKNOWN );
	ar << BYTE_SEND;
	ar << dwPlayerId;
	ar << dwObjId;
	SEND( ar, this, DPID_SERVERPLAYER );
}
void CDPClient::OnItemLink( CAr &ar )
{
	BYTE nId;
	ar >> nId;
	if (nId != BYTE_RECIEVE)
	...
	SAFE_DELETE(g_WndMng.m_pWndLinkedItem);
	g_WndMng.m_pWndLinkedItem = new CWndLinkedItem;
	g_WndMng.m_pWndLinkedItem->InsertItem(pItemElem);
	g_WndMng.m_pWndLinkedItem->Initialize();
}
#endif
...
void CDPClient::SendRandomOptionRetry(OBJID objItemId, OBJID objItemId2, int nRadio)
{
	BEFORESENDSOLE(ar, PACKETTYPE_RANDOMOPTIONRETRY, DPID_UNKNOWN);
	ar << objItemId << objItemId2 << nRadio;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif
