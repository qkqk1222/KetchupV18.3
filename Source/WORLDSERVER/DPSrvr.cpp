...
//	ON_MSG( PACKETTYPE_DO_COLLECT, &CDPSrvr::OnDoCollect );
	...
	ON_MSG(PACKETTYPE_RANDOMOPTIONRETRY, &CDPSrvr::OnRandomOptionRetry);
#endif
	...
	ON_MSG(PACKETTYPE_SORT_INVENTORY, &CDPSrvr::OnSortInventory);
#endif
}
...
void CDPSrvr::OnModifyMode( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		if( !pUser->IsAuthHigher(AUTH_GAMEMASTER) )
		{
			Error( "CDPSrvr::OnModifyMode() -> [%s] try to hack : PACKETTYPE_MODIFYMODE", pUser->GetName() );
			return;
		}
#endif // __SECURITY_FIXES
		if( f )
	...
	if( IsValidObj( pUser ) )
	{
		...
		switch( pMaterial->m_dwItemId ) 
		{
			...
			case II_SYS_SYS_ITEMMODELRETURN:
				...
				{
					if (pUser->HasActivatedEatPet() && pTarget->GetProp()->dwItemKind3 == IK3_PET)
					{
						b = FALSE;
...
#ifdef __SYS_SCR_NOR_SMELTING01
		if (pUser->HasBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING01))
		{
			if (*pAbilityOption >= 3 && *pAbilityOption < 7)
			{
				nPercent += 1000;
				pUser->RemoveBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING01);

				ItemProp* pItemProp = prj.GetItemProp(II_SYS_SYS_SCR_NOR_SMELTING01);
				if (pItemProp)
					g_dpDBClient.SendLogSMItemUse("2", pUser, NULL, pItemProp);
			}
		}
#endif
#ifdef __SYS_SCR_NOR_SMELTING02
		if (pUser->HasBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING02))
		{
			if (*pAbilityOption >= 7 && *pAbilityOption < 9)
			{
				nPercent += 500;
				pUser->RemoveBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING02);

				ItemProp* pItemProp = prj.GetItemProp(II_SYS_SYS_SCR_NOR_SMELTING02);
				if (pItemProp)
					g_dpDBClient.SendLogSMItemUse("2", pUser, NULL, pItemProp);
			}
		}
#endif
#ifdef __SYS_SCR_NOR_SMELTING03
		if (pUser->HasBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING03))
		{
			if (*pAbilityOption == 9)
			{
				nPercent += 300;
				pUser->RemoveBuff(BUFF_ITEM, II_SYS_SYS_SCR_NOR_SMELTING03);

				ItemProp* pItemProp = prj.GetItemProp(II_SYS_SYS_SCR_NOR_SMELTING03);
				if (pItemProp)
					g_dpDBClient.SendLogSMItemUse("2", pUser, NULL, pItemProp);
			}
		}
...
#else // __SECURITY_FIXES
			if( pUser->GetGold() < (int)( ( nPostGold + nGold ) ) || nGold > 2000000000) //overflow.
			{
				pUser->AddDiagText( prj.GetText( TID_GAME_LACKMONEY ) );
				return;
			}
...
#ifdef __SECURITY_FIXES
					CWorld* pWorld	= ((CUser*)pTrader)->GetWorld();
					g_dpDBClient.SavePlayer( (CUser*)pTrader, pWorld->GetID(), ((CUser*)pTrader)->GetPos(), ((CUser*)pTrader)->GetLayer() );
...
void CDPSrvr::OnReqQuestNPCPos( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	...
	QuestProp* qProp = prj.m_aPropQuest.GetAt(questId);
	if (qProp && qProp->m_szEndCondCharacter[0])
	{
		const LPCHARACTER lpChar = prj.GetCharacter(qProp->m_szEndCondCharacter);
		if (lpChar)
			pUser->AddNPCPos(lpChar->m_vPos, questId, lpChar->m_dwWorldId);
	}
	...
	const LPCHARACTER lpChar = prj.GetCharacter(szCharKey);
	if (lpChar)
	{
...
#endif
#ifdef __INVENTORY_SORTING
BOOL SortInventoryByItemKind(const vSortInv dwId, const vSortInv dwIDSec)
{
	return (dwId.dwItemKind3 < dwIDSec.dwItemKind3);
}

void CDPSrvr::OnSortInventory(CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	CUser* pUser = g_UserMng.GetUser(dpidCache, dpidUser);
	if (!IsValidObj(pUser))
		return;

	vector<vSortInv> vecSortInv;
	for (int i = 0; i < MAX_INVENTORY; i++)
	{
		CItemElem* pItemElem = (CItemElem*)pUser->m_Inventory.GetAt(i);
		if (pItemElem && IsUsableItem(pItemElem))
		{
			vSortInv pProp;
			pProp.dwItemID = pItemElem->GetProp()->dwID;
			pProp.szName = pItemElem->GetProp()->szName;
			pProp.dwItemKind3 = pItemElem->GetProp()->dwItemKind3;
			pProp.dwIndex = pItemElem->m_dwObjId;
			vecSortInv.push_back(pProp);
		}
	}

	std::sort(vecSortInv.begin(), vecSortInv.end(), SortInventoryByItemKind);
	int j = 0;
	vector<vSortInv>::iterator    iter;

	for (iter = vecSortInv.begin(); iter != vecSortInv.end(); ++iter)
	{
		CItemElem* pItemElemSwap = (CItemElem*)pUser->m_Inventory.GetAtId(iter->dwIndex);
		if (pItemElemSwap)
		{
			pUser->AddMoveItem(0, (BYTE)pItemElemSwap->m_dwObjIndex, j);
			pUser->m_Inventory.Swap(pItemElemSwap->m_dwObjIndex, j);
			j++;
		}
	}
}
...
BOOL CDPSrvr::UnbindScroll(CUser* pUser, CItemElem* pMaterial, CItemElem* pTarget)
{
	...
	if (pTarget->IsFlag(CItemElem::binds) && pMaterial->m_dwItemId == II_SYS_SYS_SCR_CANCELLATION && (pTarget->GetProp()->dwFlag & IP_FLAG_EQUIP_BIND))
	{
		...
		log.Gold2 = pUser->GetGold();
		OnLogItem(log, pTarget, 1);
		pUser->AddDefinedText(TID_GAME_SMELT_SAFETY_SUCCESS);
		return TRUE;
	}
	else
...
void CDPSrvr::OnRandomOptionRetry(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	if (IsValidObj(pUser) == TRUE)
	{
		...
		{
			if (pItemElem2->GetProp()->dwID != II_SYS_SYS_SCR_RANDOMOPTIONRTRY)
				return;
			...
			if (nRandomOptionKind == CRandomOptionProperty::eAwakening)
			{
				...
				if (nSize >= 2 && nSize <= 3)
				{
					for (int i = 0; i < nSize; i++)
					{
						g_xRandomOptionProperty->GetParam(pItemElem->GetRandomOptItemId(), i, &nDst, &nAdj);
						nDst2[i] = nDst;
						nAdj2[i] = nAdj;
					}
					...
					g_xRandomOptionProperty->GetParam(pItemElem->GetRandomOptItemId(), 0, &nDst, &nAdj);
					...
					if (nRadio == 1)
					{
						g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst3, nAdj3);
						g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst2[1], nAdj2[1]);

						if (k == 3)
						{
							g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst2[2], nAdj2[2]);
						}
					}
					else if (nRadio == 2)
					{
						g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst2[0], nAdj2[0]);
						g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst3, nAdj3);
						if (k == 3)
						{
							g_xRandomOptionProperty->SetParam(pItemElem->GetRandomOptItemIdPtr(), nDst2[2], nAdj2[2]);
					...
					pUser->UpdateItemEx((BYTE)(pItemElem->m_dwObjId), UI_RANDOMOPTITEMID, pItemElem->GetRandomOptItemId());
					pUser->UpdateItem((BYTE)(pItemElem2->m_dwObjId), UI_NUM, pItemElem2->m_nItemNum - 1);
					pUser->AddDefinedText(TID_GAME_SMELT_SAFETY_SUCCESS);
				}
			}
		}
...
void CDPSrvr::OnTreasureBoxOpen(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	if (IsValidObj(pUser))
	{
		...
		if (IsUsableItem(pBox) && IsUsableItem(pKey))
		{
			int nFound = 0;
