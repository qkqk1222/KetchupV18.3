		...
		while( nBlock && script.tok != FINISHED )
		{
			...
			if( script.Token == "SetEndCondItem" )
			{
				...
				if( script.Token ==  "," )
				{
					...
					aEndCondItem[nEndCondItemNum].m_ItemGoalData.worldId = WI_WORLD_MADRIGAL;
#endif
...
#ifdef __KIA_GOALDATA_FIX
					propQuest.m_KillNPCGoalData[nIdx].worldId = WI_WORLD_MADRIGAL;
#endif
...
#ifdef __KIA_GOALDATA_FIX
					propQuest.m_PatrolWorldGoalData.worldId = WI_WORLD_MADRIGAL;
#endif
			...
			if( script.Token == "SetEndCondCharacter" )
			{
...
#ifdef __KIA_GOALDATA_FIX
					propQuest.m_MeetCharacterGoalData.mustRequest = false;
					propQuest.m_MeetCharacterGoalData.worldId = script.GetNumber();
					script.GetToken(); // ,
#endif
				...
				{
					propQuest.m_MeetCharacterGoalData.m_dwGoalIndex = ++dwGoalIndex;
					propQuest.m_MeetCharacterGoalData.mustRequest = true;
				}
#endif
#endif // __IMPROVE_QUEST_INTERFACE
			}
...
BOOL CProject::LoadScriptDiePenalty( LPCTSTR lpszFileName )
{
	...
	if( scanner.Load( lpszFileName, FALSE ) == FALSE )
	{
		Error( "[CRITICAL] Missing File -> DiePenalty.ini" );
		return FALSE;
	}
...
BOOL CProject::LoadScriptPK( LPCTSTR lpszFileName )
{
	...
	if( scanner.Load( lpszFileName, FALSE ) == FALSE )
	{
		Error( "[CRITICAL] Missing File -> PKSetting.inc" );
		return FALSE;
	}
